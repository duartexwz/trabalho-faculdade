...
async def cadastrar_usuarios(usuarios: UsuariosSchemas, session: T_session):
    ...
    aluno_db = await session.scalar(select(Usuarios).where((Usuarios.nome == usuarios.nome) | (Usuarios.username == usuarios.username)))

    if aluno_db:
        if aluno_db.nome == usuarios.nome:
            raise HTTPException(status_code=HTTPStatus.CONFLICT, detail="Já existe um aluno cadastrado com esse nome")

        if aluno_db.username == usuarios.username:
            raise HTTPException(status_code=HTTPStatus.CONFLICT, detail="Já existe um aluno cadastrado com esse username")

    aluno_db = Usuarios(nome=usuarios.nome, username=usuarios.username, password=get_password_hash(usuarios.password), acesso=usuarios.acesso)
...
async def uptade_usuarios(usuarios_id: int, current_user: CurrentUser, session: T_session, usuarios: UsuariosUpdate):
    ...
    if usuarios.username:
        ...
        if username_em_uso:
            raise HTTPException(status_code=HTTPStatus.CONFLICT, detail="Já tem um aluno com esse username")
...
@router.delete("/{usuarios_id}", status_code=HTTPStatus.OK, response_model=Message)
